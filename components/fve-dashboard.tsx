...
type Section = 'electricity' | 'water' | 'heat' | 'economics' | 'readings';
    ...
    <main className="min-h-screen bg-background text-foreground">
      ...
      <div className="app-shell">
        <aside className="sidebar">
          ...
          <NavButton active={section === 'readings'} onClick={() => setSection('readings')} icon={<Plus />}>Nový odečet</NavButton>
        ...
        <section className="workspace">
          ...
          {section === 'economics' && <EconomicsView rows={data.electricity.filter((row) => row.period.startsWith(year))} year={year} />}
          {section === 'readings' && <ReadingsView onSaved={refresh} />}
        </section>
      </div>
    </main>
  );
...
function WaterView({ rows }: { rows: WaterReading[] }) {
  ...
  return <><div className="section-heading"><div><p className="eyebrow">Odečty</p><h2>Voda</h2></div><p>Hlavní vodoměr zahrnuje závlahu; stočné se počítá jen ze zbytku.</p></div><div className="kpi-grid three"><Kpi icon={<Droplets />} label="Hlavní vodoměr" value={latest?.mainMeterM3 != null ? `${number.format(latest.mainMeterM3)} m³` : '–'} accent="blue" /><Kpi icon={<Gauge />} label="Zálivkový vodoměr" value={latest?.gardenMeterM3 != null ? `${number.format(latest.gardenMeterM3)} m³` : '–'} accent="mint" /><Kpi icon={<TrendingDown />} label="Změna od minula" value={change != null ? `${number.format(change)} m³` : '–'} detail={latest ? `Odečet ${dateLabel(latest.measuredAt)}` : undefined} accent="navy" /></div><Card className="table-card"><CardHeader><CardTitle>Historie odečtů</CardTitle></CardHeader><CardContent><Table><TableHeader><TableRow><TableHead>Datum</TableHead><TableHead className="num">Celkem</TableHead><TableHead className="num">Závlaha</TableHead><TableHead className="num">Pro vodné a stočné</TableHead></TableRow></TableHeader><TableBody>{rows.map((row, index) => { const previous = rows[index - 1]; const total = previous && row.mainMeterM3 != null && previous.mainMeterM3 != null ? row.mainMeterM3 - previous.mainMeterM3 : null; const garden = previous && row.gardenMeterM3 != null && previous.gardenMeterM3 != null ? row.gardenMeterM3 - previous.gardenMeterM3 : null; return <TableRow key={row.id}><TableCell>{dateLabel(row.measuredAt)}</TableCell><TableCell className="num">{formatM3(total)}</TableCell><TableCell className="num">{formatM3(garden)}</TableCell><TableCell className="num strong">{formatM3(total != null && garden != null ? total - garden : null)}</TableCell></TableRow>; })}</TableBody></Table></CardContent></Card></>;
}

function EconomicsView({ rows, year }: { rows: ElectricityReading[]; year: string }) {
  const imported = rows.reduce((sum, row) => sum + (row.gridImportKwh ?? 0), 0); const net = rows.reduce((sum, row) => sum + netCost(row), 0); const consumption = rows.reduce((sum, row) => sum + (distributorConsumption(row) ?? 0), 0); const gross = rows.reduce((sum, row) => sum + (row.purchaseCostCzk ?? 0), 0); const savings = consumption * 5.5 - net;
  return <><div className="section-heading"><div><p className="eyebrow">Finanční model</p><h2>Roční ekonomika {year}</h2></div><p>Výpočty oddělují cenu nákupu, výnos z FVE a vyrovnávání sítě.</p></div><div className="kpi-grid"><Kpi icon={<CircleDollarSign />} label="Hrubá cena nákupu" value={imported ? `${decimal.format(gross / imported)} Kč/kWh` : '–'} detail="Před výnosy FVE" accent="blue" /><Kpi icon={<TrendingDown />} label="Efektivní cena" value={imported ? `${decimal.format(net / imported)} Kč/kWh` : '–'} detail="Po dodávce a vyrovnávání sítě" accent="mint" /><Kpi icon={<Sun />} label="Roční úspora" value={money.format(savings)} detail="Pro referenční cenu 5,50 Kč/kWh" accent="sun" /><Kpi icon={<Gauge />} label="Prostá návratnost" value={savings > 0 ? `${decimal.format(230000 / savings)} roku` : '–'} detail="Investice 230 000 Kč" accent="navy" /></div></>;
}

function ReadingsView({ onSaved }: { onSaved: () => Promise<void> }) {
  const [kind, setKind] = useState<'electricity-meter' | 'water' | 'heat-pump'>('electricity-meter'); const [form, setForm] = useState<Record<string, string>>({}); const [message, setMessage] = useState<string | null>(null);
  const fields = kind === 'electricity-meter' ? [['readingDate', 'Datum', 'date'], ['meterNtKwh', 'PND nízký tarif (kWh)', 'number'], ['meterVtKwh', 'PND vysoký tarif (kWh)', 'number']] : kind === 'water' ? [['measuredAt', 'Datum', 'date'], ['mainMeterM3', 'Hlavní vodoměr (m³)', 'number'], ['gardenMeterM3', 'Zálivkový vodoměr (m³)', 'number']] : [['measuredAt', 'Datum', 'date'], ['heatOutputKwh', 'Teplo topení – stav (kWh)', 'number'], ['hotWaterOutputKwh', 'Teplo TUV – stav (kWh)', 'number'], ['heatInputKwh', 'Příkon topení – stav (kWh)', 'number'], ['hotWaterInputKwh', 'Příkon TUV – stav (kWh)', 'number']];
  async function submit(event: FormEvent) { event.preventDefault(); const payload: Record<string, unknown> = {}; for (const [name, , type] of fields) payload[name] = type === 'number' ? (form[name] === '' || form[name] == null ? null : Number(form[name])) : form[name]; const response = await fetch(`/api/readings/${kind}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) }); const result = await response.json() as { error?: string }; if (!response.ok) { setMessage(result.error ?? 'Uložení se nezdařilo.'); return; } setForm({}); setMessage('Odečet byl uložen.'); await onSaved(); }
  return <><div className="section-heading"><div><p className="eyebrow">Surová data</p><h2>Přidat odečet</h2></div><p>Zadávej stavy měřidel; intervaly a COP portál dopočítá.</p></div><Card className="table-card"><CardContent><form onSubmit={submit}><div className="form-grid"><div className="field"><Label htmlFor="kind">Typ odečtu</Label><NativeSelect id="kind" value={kind} onChange={(e) => { setKind(e.target.value as typeof kind); setForm({}); }}><NativeSelectOption value="electricity-meter">Elektroměr PND</NativeSelectOption><NativeSelectOption value="water">Voda</NativeSelectOption><NativeSelectOption value="heat-pump">Tepelné čerpadlo</NativeSelectOption></NativeSelect></div>{fields.map(([name, label, type]) => <FormField key={name} label={label} name={name} type={type} value={form[name] ?? ''} onChange={(value) => setForm({ ...form, [name]: value })} required={type === 'date' || kind !== 'heat-pump'} />)}</div><DialogFooter><Button type="submit">Uložit odečet</Button></DialogFooter>{message && <p className="message">{message}</p>}</form></CardContent></Card></>;
}
