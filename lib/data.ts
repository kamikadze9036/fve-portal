...
import { appMetadata, economicsSettings, electricityMeterReadings, electricityReadings, heatPumpMeterReadings, heatPumpReadings, waterMeterReadings, waterReadings } from '@/db/schema';
...
export async function getAllData() {
  ...
  const db = getDb();
  const [electricity, water, heatPump, electricityMeter, waterMeter, heatPumpMeter, economics] = await Promise.all([
  ...
  return { electricity, water, heatPump, electricityMeter, waterMeter, heatPumpMeter, economics };
}
